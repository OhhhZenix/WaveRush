...
Game::Game()
{
	...
	m_Window = SDL_CreateWindow(m_Settings.Title.c_str(), SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
		m_Settings.Width, m_Settings.Height, SDL_WINDOW_SHOWN);
...
void Game::Run()
{
	// Init
	{
		m_EntityManager.Spawn(EntityType::BasicEnemy, 5);
	...
	// Loop
	{
		...
		while (m_Running)
		{
			...
			f_DeltaTime = static_cast<float>(f_DeltaTimer.GetTicks() / 1000.0);
			f_DeltaTimer.Start();
